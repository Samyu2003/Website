...
if(isset($_REQUEST['sv-vstr']))
{
  ...
  if($status == 0)
{
  $update_visitor = mysqli_query($conn,"update tbl_visitors set name='$name', email='$email', cno='$cno', 
  location='$location', meet='$meet', organization='$organization', , status='$status', 
  out_time=now() where id='$id'");
}
else
{
  $update_visitor = mysqli_query($conn,"update tbl_visitors set name='$name', email='$email', cno='$cno
  
  ', address='$address', to_meet='$meet', department='$department', reason='$reason', status='$status' where id='$id'");
}
...
<?php include('include/side-bar.php'); ?>
    <div id="content-wrapper">
      ...
      <label class="col-lg-4 col-form-label" for="name">Name</label>
       <div class="col-lg-6">
      <input type="text" name="name" id="name" class="form-control" placeholder="Enter Name" required value="<?php echo $row['name']; ?>" readonly>
       </div>
      </div>
      <div class="form-group row">
